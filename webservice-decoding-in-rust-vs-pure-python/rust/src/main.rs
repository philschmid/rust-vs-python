...
async fn index(
    ...
    let pred = text_classification
        .call1(gil.python(), ("i love you.",))
        .unwrap();
...
async fn main() -> std::io::Result<()> {
    ...
    let text_classification: Data<Py<PyAny>> = Data::new(
        transformers
            .getattr("pipeline")
            .unwrap()
    ...
    HttpServer::new(move || {
        App::new()
            // .app_data() // add thread-local state
            .app_data(text_classification.clone()) // add thread-local state
            .service(
                web::resource("/age")
